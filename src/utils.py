...
def get_local_dir(prefixes_to_resolve: List[str]) -> str:
    ...
    for prefix in prefixes_to_resolve:
        if os.path.exists(prefix):
            return f"{prefix}/root" # {getpass.getuser()}"
    os.makedirs(prefix)
    return f"{prefix}/root" # {getpass.getuser()}"
    

def get_local_run_dir(exp_name: str, local_dirs: List[str], datasets, phase, loss) -> str:
    """Create a local directory to store outputs for this run, and return its path."""
    # now = datetime.now()
    # timestamp = now.strftime("%Y-%m-%d_%H-%M-%S_%f")
    run_dir = f"{get_local_dir(local_dirs)}/results/{exp_name}/{datasets[0]}/{loss.name}"
